...
const EditHoliday = ({navigation}) => {
    ...
    useEffect(()=>{
        const getHoliday = async () => {
            const data = await AsyncStorage.getItem('HolidayData');
            console.log("first :- ",JSON.parse(data));
            const data1 = JSON.parse(data);
            setHoliday(data1?.HolidayName);
            setDay(data1?.Day);
            setMonth(data1?.Month);
            setYear(data1?.Year);
            setId(data1?._id);
        }
        getHoliday()
    ...
    const handleSubmit = async () => {
        ...
        await axios.put(`${Rupioo_Lite_BaseUrl}holiday/update-holiday/${Id}`,{
            userId: id,
            Year: year,
            Month: month,
            Day: day,
