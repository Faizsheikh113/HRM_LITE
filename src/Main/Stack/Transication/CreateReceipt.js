...
const Create_Receipt = ({ navigation }) => {
    const [employeeList, setEmployeeList] = useState([]);
    const [formFields, setFormFields] = useState([{ id: Date.now(), startDate: null, selectedEmployee: null, remark: '', amount: '', status: null, showDatePicker: false, employeeOpen: false, statusOpen: false }]);
    ...
    const handleAddMore = () => {
        setFormFields(prevFields => [
            ...prevFields,
            { id: Date.now(), startDate: null, selectedEmployee: null, remark: '', amount: '', status: null, showDatePicker: false, employeeOpen: false, statusOpen: false }
        ]);
    };
    ...
    const handleRemove = (id) => {
        // Prevent removal of the first field
    ...
    const validateForm = () => {
        for (const field of formFields) {
            if (!field.startDate) {
                Alert.alert('Validation Error', 'Please select a start date for each set.');
                return false;
            }
            if (!field.selectedEmployee) {
                Alert.alert('Validation Error', 'Please select an employee for each set.');
                return false;
            }
            if (!field.status) {
                Alert.alert('Validation Error', 'Please select a payment type for each set.');
                return false;
            }
            if (!field.amount || isNaN(field.amount) || parseFloat(field.amount) <= 0) {
                Alert.alert('Validation Error', 'Please enter a valid amount for each set.');
                return false;
            }
        }
        return true;
    ...
    const handleSubmit = async () => {
        ...
        const receiptData = formFields.map(field => ({
            userId:id,
            date: moment(field.startDate).format('YYYY-MM-DD'),
            employeeId: field.selectedEmployee?._id,
            remark: field.remark,
            amount: parseFloat(field.amount),
            paymentMode: field.status,
            type:'receipt'
        }));
        console.log(receiptData);
        await axios.post(`${Rupioo_Lite_BaseUrl}receipt/save-receipt`,{Receipt:receiptData})
            .then(async(res) => {
                        ...
                        {formFields.map((field, index) => (
                            <View key={field.id} style={styles.fieldContainer}>
                                ...
                                <DropDownPicker
                                    ...
                                    // setValue={value => handleFieldChange(field.id, 'status', value)}
